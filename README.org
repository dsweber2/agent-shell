...
For [[https://docs.letta.com/letta-agent][Letta Code]] (stateful agents with long-term memory), install the [[https://tangled.org/codeluggage.tngl.sh/letta-code-acp][letta-code-acp]] adapter:
#+begin_src bash
npm install letta-code-acp
#+end_src
Or from source
#+begin_src bash
git clone https://tangled.org/codeluggage.tngl.sh/letta-code-acp
cd letta-code-acp
npm install && npm run build
#+end_src

You also need the ~letta~ CLI on your ~$PATH~. See [[https://docs.letta.com/letta-agent/quickstart][letta-ai/letta-code]] for install instructions.

Authentication uses either a Letta Cloud API key (~LETTA_API_KEY~) or a connected local provider (~letta connect <provider>~ run once outside Emacs).
...
Letta agents are stateful — they persist long-term memory across sessions and conversations. Configuration is driven entirely through environment variables set on ~agent-shell-letta-environment~; the adapter reads them at process start.

If you built ~letta-code-acp~ from source (see [[*Letta Code][External dependencies → Letta Code]]), point ~agent-shell-letta-acp-command~ at the local binary. Use ~expand-file-name~ — tilde won't expand inside a process command list:
...
Then configure the adapter environment. Common variables:

- ~LETTA_API_KEY~ — Letta Cloud credentials (omit for a connected local provider).
- ~LETTA_AGENT_ID~ — pin all shells to a specific agent (~agent-...~).
- ~LETTA_MODEL~ — model handle (e.g. ~auto~).
- ~LETTA_PERMISSION_MODE~ — one of ~standard~, ~accept-edits~, ~bypass-permissions~.

For Letta Cloud, load ~LETTA_API_KEY~ from a ~.env~ file and inherit shell ~PATH~ so ~letta~ resolves:
...
(setq agent-shell-letta-environment
      (agent-shell-make-environment-variables
       ...
       "LETTA_MODEL" "auto"
       "LETTA_PERMISSION_MODE" "standard"))
#+end_src

For a local Letta backend (no cloud key), just inherit the environment:
...
Because Letta agents are stateful, ~agent-shell-letta.el~ provides two entry points beyond the standard ~agent-shell-letta-start-agent~:

- =M-x agent-shell-letta-start-main-chat= — attach to the agent's persistent top-level conversation (renameable, not deletable). Multiple shells opened with this command share the same conversation and memory.
- =M-x agent-shell-letta-start-conversation= — spawn a fresh, isolated conversation under the same agent (separate history, still shares agent-level memory).
...
- =M-x agent-shell-letta-start-agent= - Start a Letta Code agent session
- =M-x agent-shell-letta-start-main-chat= - Attach to the Letta agent's persistent main chat
- =M-x agent-shell-letta-start-conversation= - Spawn a fresh Letta Code conversation
